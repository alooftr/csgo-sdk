...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    static std::map<ImGuiID, color_animation_t> anim_map;
    auto anim = anim_map.find( ImColor( ) );

    if ( anim == anim_map.end( ) )
    {
        anim_map.insert( { ImGui::GetItemID( ), color_animation_t( ) } );
        anim = anim_map.find( ImGui::GetItemID( ) );
    }

    anim->second.active = ImLerp( anim->second.active, hovered || g.ActiveId == id ? ImColor( GetColorU32( ImGuiCol_CheckMark ) ).Value : ImColor( GetColorU32( ImGuiCol_TextDisabled ) ).Value, g.IO.DeltaTime * style.AnimationScale );
    anim->second.bg = ImLerp( anim->second.bg, ( held && hovered ) ? ImColor( GetColorU32( ImGuiCol_FrameBgActive ) ).Value : hovered ? ImColor( GetColorU32( ImGuiCol_FrameBgHovered ) ).Value : ImColor( GetColorU32( ImGuiCol_FrameBg ) ).Value, g.IO.DeltaTime * style.AnimationScale );


    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);
    RenderFrame(check_bb.Min, check_bb.Max, anim->second.bg, true, style.FrameRounding);
    ImU32 check_col = anim->second.active;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    anim->second.bg = ImLerp( anim->second.bg, ImColor( frame_col ).Value, g.IO.DeltaTime * style.AnimationScale );

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    RenderNavHighlight(bb, id);
    if (!(flags & ImGuiComboFlags_NoPreview))
        window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2, bb.Max.y), anim->second.bg, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersLeft);
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        if (value_x2 + arrow_size - style.FramePadding.x <= bb.Max.x)
            // modified arrow keys up/down on open/close
            RenderArrow(window->DrawList, ImVec2(value_x2 + style.FramePadding.y, bb.Min.y + style.FramePadding.y), text_col, popup_open ? ImGuiDir_Down : ImGuiDir_Up, 1.0f);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    anim->second.active = ImLerp( anim->second.active, hovered || g.ActiveId == id ? ImColor( GetColorU32( ImGuiCol_SliderGrabActive ) ).Value : ImColor( GetColorU32( ImGuiCol_SliderGrab ) ).Value, g.IO.DeltaTime * style.AnimationScale );
    anim->second.bg = ImLerp( anim->second.bg, hovered || g.ActiveId == id ? ImColor( GetColorU32( ImGuiCol_FrameBgActive ) ).Value : ImColor( GetColorU32( ImGuiCol_FrameBg ) ).Value, g.IO.DeltaTime * style.AnimationScale );

    // Draw frame
    RenderNavHighlight(frame_bb, id);
    RenderFrame(frame_bb.Min, frame_bb.Max, anim->second.bg, true, g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, anim->second.active, style.GrabRounding );
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    anim->second.bg = ImLerp( anim->second.bg, ImColor( GetColorU32( ( held && hovered ) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered : ImGuiCol_Header ) ).Value, g.IO.DeltaTime * style.AnimationScale );

    // Render
    if (hovered || selected)
    {
        RenderFrame( bb.Min, bb.Max, anim->second.bg, false, 0.0f );
        // gradient
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    anim->second.bg = ImLerp( anim->second.bg, ImColor( tab_col ).Value, g.IO.DeltaTime * style.AnimationScale );

    const ImU32 text_col = GetColorU32( ( held || hovered ) ? ImGuiCol_TabHovered : tab_contents_visible ? ( tab_bar_focused ? ImGuiCol_TabActive : ImGuiCol_TabUnfocusedActive ) : ImGuiCol_TextDisabled );
    anim->second.active = ImLerp( anim->second.active, ImColor( text_col ).Value, g.IO.DeltaTime * style.AnimationScale );

    TabItemBackground(display_draw_list, bb, flags, anim->second.bg );
    RenderNavHighlight(bb, id);
    ...
    bool just_closed;
    bool text_clipped;
    PushStyleColor( ImGuiCol_Text, anim->second.active.Value );
