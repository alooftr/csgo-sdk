...
namespace csys
{
    ...
    public:
         ...
         */
        ItemLog &Log(ItemType type = ItemType::TYPE_LOG);
        ...
        void RegisterCommand(const String &name, const String &description, Fn function, Args... args)
        {
            ...
            {
                Log( TYPE_ERROR) << "Empty command name given" << csys::endl;
                return;
            }
            ...
            auto help = [this, command_name]() {
                Log( TYPE_LOG) << m_Commands[command_name]->Help() << csys::endl;
            };
    ...
    protected:
        ...
        std::string RegisterVariableAux(const String &name, T &var)
        {
            ...
            auto range = name.NextPoi(name_index);
            if (name.NextPoi(name_index).first != name.End())
                throw csys::Exception( "ERROR: Whitespace separated variable names are forbidden " + name.m_String );
            ...
            {
                m_ItemLog.log( TYPE_LOG ) << var << endl;
            };
