...
namespace csys
{
    ...
    CSYS_INLINE void System::RunCommand(const std::string &line)
    {
        ...
        // Log command.
        Log(csys::ItemType::TYPE_COMMAND) << line << csys::endl;
    ...
    CSYS_INLINE void System::RunScript(const std::string &script_name)
    {
        ...
        {
            m_ItemLog.log( TYPE_ERROR) << "Script \"" << script_name << "\" not found" << csys::endl;
            return;
        }

        // About to run script.
        m_ItemLog.log( TYPE_INFO) << "Running \"" << script_name << "\"" << csys::endl;
        ...
        if (script_pair->second->Data().empty())
        {
            ...
            {
                Log( TYPE_ERROR) << e.what() << csys::endl;
            }
        }
    ...
    CSYS_INLINE void System::ParseCommandLine(const String &line)
    {
        ...
        else if (is_cmd_set || is_cmd_get)
        {
            ...
            {
                Log( TYPE_ERROR) << s_ErrorNoVar << endl;
                return;
            } else
        ...
        if (command == m_Commands.end())
            Log( TYPE_ERROR) << s_ErrorSetGetNotFound << endl;
            // Run the command
        else
        {
            // Get the arguments.
