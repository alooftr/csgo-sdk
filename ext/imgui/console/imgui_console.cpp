...
void ImGuiConsole::Draw()
{
    ...
    if (!ImGui::Begin(m_ConsoleName.data(), nullptr))
    {
        ImGui::PopStyleVar();
        ImGui::End();
        return;
...
void ImGuiConsole::LogWindow()
{
    ...
    if (ImGui::BeginChild("ScrollRegion##", ImVec2(0, -footerHeightToReserve), false, 0))
    {
        ...
        for (const auto &item : m_ConsoleSystem.Items())
        {
            ...
            if (item.m_Type == csys::TYPE_COMMAND)
            {
            ...
            // Time stamp.
            if (item.m_Type == csys::TYPE_COMMAND && m_TimeStamps)
            {
    ...
    switch (data->EventFlag)
    {
        case ImGuiInputTextFlags_CallbackCompletion:
        {
            ...
            if (!trim_str.empty())
            {
                ...
                {
                    console->m_ConsoleSystem.Log(csys::TYPE_COMMAND) << "Suggestions: " << csys::endl;

                    for (const auto &suggestion : console->m_CmdSuggestions)
                        console->m_ConsoleSystem.Log(csys::TYPE_LOG) << suggestion << csys::endl;
