...
bool g_math::setup( )
{
	random_seed = reinterpret_cast< random_seed_fn >( g_game_modules->get( vstdlib_dll ).get_proc_address( "RandomSeed" ) );
	if ( random_seed == nullptr )
		return false;

	random_float = reinterpret_cast< random_float_fn >( g_game_modules->get( vstdlib_dll ).get_proc_address( "RandomFloat" ) );
	if ( random_float == nullptr )
		return false;

	random_float_exp = reinterpret_cast< random_float_exp_fn >( g_game_modules->get( vstdlib_dll ).get_proc_address( "RandomFloatExp" ) );
	if ( random_float_exp == nullptr )
		return false;

	random_int = reinterpret_cast< random_int_fn >( g_game_modules->get( vstdlib_dll ).get_proc_address( "RandomInt" ) );
	if ( random_int == nullptr )
		return false;

	random_gaussian_float = reinterpret_cast< random_gaussian_float_fn >( g_game_modules->get( vstdlib_dll ).get_proc_address( "RandomGaussianFloat" ) );
	if ( random_gaussian_float == nullptr )
		return false;

	debug_log_ok( "finished setup math library" );

	return true;
}

void g_math::transform_vector( const vec3_t& a, const matrix3x4_t& b, vec3_t& out )
{
...
void g_math::angle_vectors( const vec3_t& view, vec3_t* forward, vec3_t* right, vec3_t* up )
{
...
void g_math::vector_angles( const vec3_t& forward, vec3_t& view )
{
	float pitch, yaw;
...
float g_math::normalize_yaw( float yaw )
{
	if ( !std::isfinite( yaw ) )
		return 0.0f;
...
vec3_t g_math::vector_transform( const vec3_t& transform, const matrix3x4_t& matrix )
{
...
void g_math::vector_transform( const vec3_t& in, const matrix3x4_t& matrix, vec3_t& out )
{
	out =
	{
...
vec3_t g_math::calc_angle( const vec3_t start, vec3_t end )
{
	vec3_t delta = vec3_t(
		( start.x - end.x ),
		( start.y - end.y ),
...
float g_math::get_fov( const vec3_t& view_angle, const vec3_t& aim_angle )
{
...
bool g_math::world_to_screen( const vec3_t& origin, vec2_t& screen )
{
	const view_matrix_t& matrix = g_game_interfaces->engine->world_to_screen_matrix( );
...
void g_math::angle_matrix( const vec3_t& angView, matrix3x4_t& matOutput, const vec3_t& vecOrigin )
{
	float sp, sy, sr, cp, cy, cr;
...
void g_math::angle_matrix( const vec3_t& ang, const vec3_t& pos, matrix3x4_t& out )
{
	angle_matrix( ang, out, vec3_t( 0.0f, 0.0f, 0.0f ) );
	out.set_origin( pos );
}

void g_math::matrix_copy( const matrix3x4_t& in, matrix3x4_t& out )
{
	memcpy( out.base( ), in.base( ), sizeof( float ) * 3 * 4 );
}

void g_math::concat_transforms( const matrix3x4_t& in1, const matrix3x4_t& in2, matrix3x4_t& out )
{
	if ( &in1 == &out )
	{
		matrix3x4_t in1b;
...
vec3_t g_math::calculate_relative_angle( const vec3_t& src, const vec3_t& dst, const vec3_t& view_angle )
{
	return ( ( dst - src ).to_angle( ) - view_angle ).normalize( );
}