...
public:
	...
	{
		return c_memory::call_vfunc<wchar_t*>( this, 11, token );
	}

	const wchar_t* find_safe( const char* token )
	{
		return c_memory::call_vfunc<wchar_t*>( this, 12, token );
	}

	int convert_unicode_to_ansi( const wchar_t* unicode, char* ansi, int ansi_buffer_size )
	{
		return c_memory::call_vfunc<int>( this, 16, unicode, ansi, ansi_buffer_size );
	}
