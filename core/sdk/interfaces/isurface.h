...
public:
	...
	{
		c_memory::call_vfunc<void>( this, 14, clr );
	}

	void draw_set_color( int r, int g, int b, int a )
	{
		c_memory::call_vfunc<void>( this, 15, r, g, b, a );
	}

	void draw_filled_rect( int x0, int y0, int x1, int y1 )
	{
		c_memory::call_vfunc<void>( this, 16, x0, y0, x1, y1 );
	}

	void draw_filled_rect_fade( int x0, int y0, int x1, int y1, uint32_t alpha1, uint32_t alpha2, bool horizontal )
	{
		c_memory::call_vfunc<void>( this, 123, x0, y0, x1, y1, alpha1, alpha2, horizontal );
	}

	void draw_outlined_rect( int x0, int y0, int x1, int y1 )
	{
		c_memory::call_vfunc<void>( this, 18, x0, y0, x1, y1 );
	}

	void draw_line( int x0, int y0, int x1, int y1 )
	{
		c_memory::call_vfunc<void>( this, 19, x0, y0, x1, y1 );
	}

	void draw_poly_line( int* x, int* y, int points )
	{
		c_memory::call_vfunc<void>( this, 20, x, y, points );
	}

	void draw_set_text_font( h_font font )
	{
		c_memory::call_vfunc<void>( this, 23, font );
	}

	void draw_set_text_color( col_t col_t )
	{
		c_memory::call_vfunc<void>( this, 24, col_t );
	}

	void draw_set_text_color( int r, int g, int b, int a )
	{
		c_memory::call_vfunc<void>( this, 25, r, g, b, a );
	}

	void draw_set_text_pos( int x, int y )
	{
		c_memory::call_vfunc<void>( this, 26, x, y );
	}

	void draw_print_text( const wchar_t* text, int text_length, e_font_draw_type draw_type = font_draw_default )
	{
		c_memory::call_vfunc<void>( this, 28, text, text_length, draw_type );
	}

	void draw_set_texture_rgba( int index, const unsigned char* rgba, int wide, int iTall )
	{
		c_memory::call_vfunc<void>( this, 37, index, rgba, wide, iTall );
	}

	void draw_set_texture( int index )
	{
		c_memory::call_vfunc<void>( this, 38, index );
	}

	int create_new_texture_id( bool procedural = false )
	{
		return c_memory::call_vfunc<int>( this, 43, procedural );
	}

	void unlock_cursor( )
	{
		return c_memory::call_vfunc<void>( this, 66 );
	}

	void lock_cursor( )
	{
		return c_memory::call_vfunc<void>( this, 67 );
	}

	h_font create_font( )
	{
		return c_memory::call_vfunc<h_font>( this, 71 );
	}
	...
	{
		return c_memory::call_vfunc<bool>( this, 72, font, windows_font_name, tall, weight, blur, scan_lines, flags, range_min, range_max );
	}

	void get_text_size( h_font font, const wchar_t* text, int& wide, int& tall )
	{
		c_memory::call_vfunc<void>( this, 79, font, text, std::ref( wide ), std::ref( tall ) );
	}

	void play_sound_surface( const char* file_name )
	{
		c_memory::call_vfunc<void>( this, 82, file_name );
	}

	void draw_outlined_circle( int x, int y, int radius, int segments )
	{
		c_memory::call_vfunc<void>( this, 103, x, y, radius, segments );
	}

	void draw_textured_polygon( int n, vertex_t* vertice, bool clip_vertices = true )
	{
		c_memory::call_vfunc<void>( this, 106, n, vertice, clip_vertices );
	}
};