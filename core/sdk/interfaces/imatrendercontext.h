...
public:
	void begin_render( )
	{
		c_memory::call_vfunc<void>( this, 2 );
	}

	void end_render( )
	{
		c_memory::call_vfunc<void>( this, 3 );
	}

	void bind_local_cubemap( i_texture* texture )
	{
		c_memory::call_vfunc<void>( this, 5, texture );
	}

	void set_render_target( i_texture* texture )
	{
		c_memory::call_vfunc<void>( this, 6, texture );
	}

	i_texture* get_render_target( )
	{
		return c_memory::call_vfunc<i_texture*>( this, 7 );
	}

	void clear_buffers( bool clear_color, bool clear_depth, bool clear_stencil = false )
	{
		c_memory::call_vfunc<void>( this, 12, clear_color, clear_depth, clear_stencil );
	}

	void set_lights( int count, const light_desc_t* lights )
	{
		c_memory::call_vfunc<void>( this, 17, count, lights );
	}

	void set_ambient_light_cube( vec4_t cube[ 6 ] )
	{
		c_memory::call_vfunc<void>( this, 18, cube );
	}

	void viewport( int x, int y, int w, int h )
	{
		c_memory::call_vfunc<void>( this, 40, x, y, w, h );
	}

	void get_viewport( int& x, int& y, int& w, int& h )
	{
		c_memory::call_vfunc<void>( this, 41, std::ref( x ), std::ref( y ), std::ref( w ), std::ref( h ) );
	}

	void clear_color3ub( unsigned char r, unsigned char g, unsigned char b )
	{
		c_memory::call_vfunc<void>( this, 78, r, g, b );
	}

	void clear_color4ub( unsigned char r, unsigned char g, unsigned char b, unsigned char a )
	{
		c_memory::call_vfunc<void>( this, 79, r, g, b, a );
	}
	...
	{
		c_memory::call_vfunc<void>( this, 114, material, dest_x, dest_y, w, h, texture_x0, texture_y0, texture_x1, texture_y1, texture_width, texture_height, client_renderable, x_dice, y_dice );
	}

	void push_render_target_and_viewport( )
	{
		c_memory::call_vfunc<void>( this, 119 );
	}

	void pop_render_target_and_viewport( )
	{
		c_memory::call_vfunc<void>( this, 120 );
	}

	void set_int_rendering_parameter( int parm_number, int value )
	{
		c_memory::call_vfunc<void>( this, 126, parm_number, value );
	}

	void set_lighting_origin(/*vec3_t vecLightingOrigin*/float x, float y, float z )
	{
		c_memory::call_vfunc<void>( this, 158, x, y, z );
	}

	void set_scissor_rect( const int left, const int top, const int right, const int bottom, const bool enable_scissor = true )
	{
		c_memory::call_vfunc<void>( this, 159, left, top, right, bottom, enable_scissor );
	}

	void pop_scissor_rect( )
	{
		c_memory::call_vfunc<void>( this, 160 );
	}
};
