...
public:
	...
	c_trace_filter( const i_handle_entity* skip_ent, e_trace_type trace_type = trace_everything )
		: skip( skip_ent ), trace_type( trace_type )
	{ }

	c_trace_filter( filter_callback_fn&& check_callback, e_trace_type trace_type = trace_everything )
		: check_callback( std::move( check_callback ) ), trace_type( trace_type )
	{ }

	bool should_hit_entity( i_handle_entity* handle_entity, int contents_mask ) override
	{
		// if given user defined callback - check it
		if ( check_callback != nullptr )
			return check_callback( handle_entity, contents_mask );

		// otherwise skip entity if given
		return handle_entity != skip;
	}
...
public:
	...
	virtual void clip_ray_to_collideable( const ray_t& ray, unsigned int mask, i_collideable* pCollide, trace_t* trace ) = 0;
	virtual void trace_ray( const ray_t& ray, unsigned int mask, i_trace_filter* trace_filter, trace_t* trace ) = 0;
	virtual void setup_leaf_and_entity_list_ray( const ray_t& ray, i_trace_list_data* trace_data ) = 0;
	virtual void setup_leaf_and_entity_list_box( const vec3_t& box_min, const vec3_t& box_max, i_trace_list_data* trace_data ) = 0;
	virtual void trace_ray_against_leaf_and_entity_list( const ray_t& ray, i_trace_list_data* trace_data, unsigned int mask, i_trace_filter* trace_filter, trace_t* trace ) = 0;
	virtual void sweep_collideable( i_collideable* collide, const vec3_t& abs_start, const vec3_t& abs_end, const vec3_t& angles, unsigned int mask, i_trace_filter* trace_filter, trace_t* trace ) = 0;
	virtual void enumerate_entities( const ray_t& ray, bool triggers, i_entity_enumerator* enumerator ) = 0;
	virtual void enumerate_entities( const vec3_t& abs_mins, const vec3_t& abs_maxs, i_entity_enumerator* enumerator ) = 0;
	virtual i_collideable* get_collideable( i_handle_entity* entity ) = 0;
	virtual int get_stat_by_index( int index, bool clear ) = 0;
	virtual void get_brushes_in_aabb( const vec3_t& mins, const vec3_t& maxs, c_utl_vector<int>* output, int contents_mask = MASK_ALL ) = 0;
	virtual c_phys_collide* get_collidable_from_displacements_in_aabb( const vec3_t& mins, const vec3_t& maxs ) = 0;
	virtual int get_num_displacements( ) = 0;
	virtual void get_displacement_mesh( int index, virtualmeshlist_t* mesh_tri_list ) = 0;
	virtual bool get_brush_info( int brush, c_utl_vector<brush_side_info_t>* brush_side_info_out, int* contents_out ) = 0;
	...
	virtual void free_trace_list_data( i_trace_list_data* list_data ) = 0;
	virtual int get_set_debug_trace_counter( int value, e_debug_trace_counter_behavior behavior ) = 0;
	virtual int get_meshes_from_displacements_in_aabb( const vec3_t& mins, const vec3_t& maxs, virtualmeshlist_t* output_meshes, int max_output_meshes ) = 0;
	virtual void get_brushes_in_collideable( i_collideable* collideable, c_brush_query& brush_query ) = 0;
	virtual bool is_fully_occluded( int occlusion_key, const aabb_t& aabb1, const aabb_t& aabb2, const vec3_t& shadow ) = 0;
